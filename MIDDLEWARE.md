...
- **Framework**: Works with any HTTP framework that supports standard middleware patterns
- **Key Generation**: Multiple built-in strategies for cache key generation (URL-based, user-specific, custom headers)
- **Pattern-Based Invalidation**: Wildcard pattern support for selective cache invalidation
- **Policies**: Content-type based caching, size-based filtering, and custom cache policies
- **Monitoring**: Hit/miss callbacks and statistics
- **Eviction Strategies**: LRU, LFU, FIFO, or AdmissionLFU
...
// CRITICAL: Set path-based key generator and path extractor
middleware.SetKeyGenerator(cache.KeyWithoutQuery())
